...
OUTPUT_ZPK = BASE / "Neumorphism_watchface_counterclockwise.zpk"
...
def build_zpk() -> None:
    ...
    print("device.zip entries:")
    with zipfile.ZipFile(DEVICE_ZIP) as zf:
        for info in zf.infolist()[:5]:
            print(f"  {info.filename}")
        print(f"  ... total {len(zf.infolist())} entries")
    print("zpk entries:")
    with zipfile.ZipFile(OUTPUT_ZPK) as zf:
